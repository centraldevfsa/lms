<template>
  <div v-if="user" :class="['dashboard flex min-h-screen', darkMode ? 'bg-gray-900 text-gray-200' : 'bg-gray-100 text-gray-800']">
    <!-- Menu lateral -->
    <aside :class="[darkMode ? 'bg-gray-800' : 'bg-white']" class="w-64 shadow-lg">
      <div class="p-6 flex justify-between items-center">
        <h1 class="text-xl font-bold">{{ user.role === 'admin' ? 'Admin Dashboard' : 'Aluno Dashboard' }} </h1>
    ...
    <main class="flex-1 p-6">
      ...
      <div class="mb-6">
        ...
        <p class="mt-2 text-sm text-gray-500">Você completou {{ user.progress }}% de seus cursos.</p>
      </div>
      ...
      <section class="mb-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div v-for="course in user.courses" :key="course.id"
            ...
            <router-link :to="`/cursos/${course.id}`" class="block mt-2 text-blue-500 hover:underline">
              Acessar Curso
            </router-link>
          </div>
        </div>
      ...
      <section class="mb-8">
        ...
        <div class="bg-yellow-100 rounded-lg shadow-md p-4 flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-sm text-gray-600">Você ganhou uma medalha por completar 50% do curso "Nutrição Funcional".
            </p>
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <router-link to="/suporte" class="block bg-white rounded-lg shadow-md p-4 hover:shadow-lg transition">
            ...
            <p class="text-sm text-gray-600">Entre em contato com nossa equipe.</p>
          </router-link>
